...
## v0.8.179 (2026-06-14) ソロモンの封印ブロック内保存を出現面変更に追従
- ソロモンの封印の出現面を変更した後、移動先ステージでブロック内配置にした場合もテストプレイ/保存ROMへ反映されるようにした。

## v0.8.178 (2026-06-14) ソロモンの封印出現面変更時の位置補正を追加
- ソロモンの封印の出現面を変更したとき、移動先の同座標に置けない場合は近くの空気マスへ自動補正するようにした。
- 移動先ステージに配置可能な空気マスが無い場合は、警告を出して変更を適用しないようにした。

## v0.8.177 (2026-06-14) ソロモンの封印と扉の重なりを禁止
- ソロモンの封印をCtrlドラッグで移動するとき、扉の位置へ重ねられないようにした。

## v0.8.176 (2026-06-14) ソロモンの封印移動時の配置制限を修正
- ソロモンの封印をCtrlドラッグで移動するとき、重ねられないブロック上へ置けないようにした。

## v0.8.175 (2026-06-14) ウィンドウ状態復元ログを通常停止
- ウィンドウサイズ・位置の診断ログを、必要な時だけ有効化する方式に変更した。

## v0.8.174 (2026-06-14) ウィンドウ状態復元ログを追加
- ウィンドウサイズ・位置の保存/復元状態を調査するため、診断ログを出力するようにした。

## v0.8.173 (2026-06-14) ソロモンの封印出現面の保存可能候補を反映
- ソロモンの封印の出現面設定で、ROM保存できる候補だけを表示するようにした。

## v0.8.172 (2026-06-14) ソロモンの封印出現面の候補計算を修正
- ソロモンの封印の出現面設定で、封印7・8の候補が狭くなりすぎる問題を修正した。

## v0.8.171 (2026-06-14) ソロモンの封印出現面の順序制約を調整
- ソロモンの封印の出現面設定で、封印1〜8の順序を保つ候補だけを表示するようにした。

## v0.8.170 (2026-06-14) ソロモンの封印出現面の範囲計算を修正
- ソロモンの封印の出現面設定で、特殊処理の範囲計算を修正し、選べるべき面が候補に出るようにした。

## v0.8.169 (2026-06-14) ソロモンの封印前半の出現面候補を調整
- ソロモンの封印1〜4の出現面設定で、1〜20面内の順序を保つ候補だけを表示するようにした。

## v0.8.168 (2026-06-14) ソロモンの封印の出現面候補を修正
- ソロモンの封印の出現面設定で、選択できる面の範囲を見直した。

## v0.8.167 (2026-06-14) ソロモンの封印の出現面候補を絞り込み
- ソロモンの封印の出現面設定で、現在の選択状態に合わせて選べる面だけを表示するようにした。

## v0.8.166 (2026-06-14) ソロモンの封印の出現面設定を追加
- ゲーム挙動改造に、ソロモンの封印8個の出現面を変更する設定を追加した。
- 1面につき封印1個までとし、20面までに4個以上、44面までに6個以上、48面までに8個配置されるよう検証する。
