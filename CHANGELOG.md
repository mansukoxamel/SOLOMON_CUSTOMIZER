...
## v0.9.56 (2026-07-30) タイトル文字にハートとアポストロフィーを追加
- タイトル画面の追加文字とPUSH START位置の固定文字で、ハート `♥`（タイトルタイル値 `$2B`／通し番号 `$72B`）を入力・保存・再読込できるようにした。
- 半角アポストロフィー `'`（タイトルタイル値 `$3B`）を追加し、既存のダブルクォート `"`（`$28`）とは別の文字として扱うようにした。
- タイトル文字編集画面の日英案内へ、追加した2文字を反映した。

## v0.9.55 (2026-07-26) 保存失敗時のデータ移行案内を全原因へ拡張
- ROM保存、終了時自動保存、テストプレイ準備などで保存に失敗した場合、runtime形式、容量制約、ディスク障害などの原因を限定せず、読込済みROMとステージがあればデータ移行を選べるようにした。
- 旧runtimeに限定していた案内文を日英とも一般化し、移行元ROMを変更しないことと、保存失敗の原因によっては移行後の保存も失敗する可能性を明記した。

## v0.9.54 (2026-07-26) ピッカーとキャンバスの名称を日英で統一
- 日本語版ピッカー191件を正式な表示名として整理し、日本語キャンバスのホバーポップアップにも同じ名称を反映した。
- 北米版公式説明書の表記を基準に英語版ピッカー191件を整備し、英語キャンバスのホバーポップアップにも反映した。
- Spark Trail `$D8-$DF` のゲーム内本体色をSPR #2へ固定し、ピッカーとキャンバス表示も同じパレットに統一した。
- 新敵共通animation入口を8バイト拡張し、PRG0使用範囲を`0x3CFE-0x3D7A`（125B）、直後の空きを`0x3D7B-0x3D9C`（34B）とした。RAM追加使用なし。

## v0.9.53 (2026-07-25) Spark Trailと方向別Spark Ballを追加
- 開発者モードへ、移動した空間をひび割れブロックで埋めるSpark Trailを追加し、アイテムをブロック内へ取り込む処理と、主人公・敵・ファイアボールがいる位置への生成防止を実装した。
- 新敵ID `$F8-$FF` を追加し、前半4IDを低速、後半4IDを高速として、初期方向ごとの壁衝突後の旋回方向を保持するようにした。
- 追加敵IDのAI、setup、init、animation共通入口と原作entity物理ラッパーを最適化し、既存IDの分類先と処理結果を維持したまま実行負荷を削減した。
- ROM/RAM管理簿、追加敵登録、runtime予約の中央整合性検査を更新し、ROM領域の重複と登録漏れを自動検査するようにした。

## v0.9.52 (2026-07-21) 16x16ピクセル編集とROMフレーム表示を改善
- 16x16ピクセル編集へ、別キャラクター・別CHRバンク・別編集画面間で共有できるコピー／貼り付けと、現在画像を全CHRバンクの同じ位置へ一括コピーする機能を追加した。
- 左半分を基準に右半分へ鏡像コピーする操作と、右半分を基準に左半分へ鏡像コピーする操作を追加し、どちらも元に戻す／やり直しへ対応した。
- スプライトビューアーと16x16編集画面でCHRバンク番号が見切れないようにし、左側8x16スプライトのSPR #1／#2を逆に表示していたパレット属性のビット順を修正した。
- ROMフレーム抽出を共通化し、間接表の4つのtype経路、9フレーム目、導入済み追加runtimeの独自フレームを一覧とタイトルキャラクター選択へ反映した。
- 全CHRバンクへの一括書込みが途中で失敗した場合、変更対象CHRタイルを実行前へ戻すようにした。

## v0.9.51 (2026-07-20) スプライトビューアの操作欄を復旧
- v0.9.49で横スクロール対応した際に表示されなくなった、スプライトビューア上部のバンク、パレット、拡大率などの操作欄を再表示するようにした。
- ROMフレーム表示と生CHR表示を切り替えた後も、プルダウンなどの操作部品が正しい大きさで表示されることを自動検査するようにした。

## v0.9.50 (2026-07-20) 旧版ROMのデータ移行と自動再読込
- 旧SOLOMON_CUSTOMIZER製ROMがruntime署名不一致で保存前検査に失敗した場合、明示保存、終了時自動保存、テストプレイ準備などのエラー画面からデータ移行へ進めるようにした。ディスク障害や容量制約など、旧runtimeと無関係な失敗では案内しない。
- 現在のアプリが認識できているステージ、共通設定、タイトル編集データ、CHR画像を、原作ROMから作る現行形式のROMへ移すようにした。認識できない旧設定は現行既定値とし、移行元ROMは上書きしない。
- 移行完了時に既存の作業状態自動保存を作成し、その自動保存ROMを通常経路で再読込することで、移行後のCHR・設定・描画・ピッカーをすべて作り直すようにした。

## v0.9.49 (2026-07-20) 画像からステージのブロック配置を生成
- PNG、JPEGなどの画像を15×12の空気・茶壁・白壁・ひび壁へ変換し、メイン画面のボタンまたは画像のドラッグ＆ドロップから現在のステージへ適用できるようにした。
- 4種類のブロック個数を合計180個の連動スライダーで調整し、画像読込時の自然配分、各45個への均等化、元画像からの読み直し、同一輝度を含む正確な個数割当へ対応した。
- 範囲選択の枠操作に応じてカーソル形状を変え、通常モードから画像をドラッグした場合も範囲選択へ切り替えるようにした。
- 画面外にあるダイアログ位置の復元防止、横幅が大きすぎるダイアログ、日本語・英語表記を整理し、メイン画面左側の最後の2つの編集ボタンを横並びにした。
- US版原作ROMとして確認済みのCRC32を追加し、未知のヘッダーでもPRGとCHRの両方が一致すれば同じ正規化処理へ進む判定を追加した。

## v0.9.48 (2026-07-20) US版原作ROMの正規化読込に対応
- 確認済みのUS版原作ROMを読み込み時に日本版相当の論理配置へ正規化し、日本版と同じmapper66 / wide-title拡張・編集経路で扱えるようにした。`.nes`直接読込と`.zip`内ROMの両方に対応する。
- 改造済みUS mapper3 ROMとskchain系US66 ROMを閲覧専用で受け付け、ステージ出力、テストプレイ、データ移行へ接続した。33本のUS66改造ROMを含む各入力経路の回帰テストを追加した。
- タイトル画面はROM読込時の正規化結果を直接編集する方式へ統一し、別ROMからタイトル画面だけを移植する旧機能とUIを削除した。
- Spark Ballの速度選択肢を安全確認済みの0.5倍・1.0倍に限定し、壁沿い移動が破綻する1.5倍・2.0倍を選択できないようにした。

## v0.9.47 (2026-07-20) 強化敵の時間処理・再設定・既定値を改善
- Dark Fairyの毒進行を敵AIの呼出し回数ではなく画面フレームで計測し、敵数によって進行時間が変わらないようにした。
- 強化Spark Ballの設定を一度適用した後も、既存runtimeを正しく検証して再設定できるようにした。
- Ghost A-F、Spark Ball反転、Neul A/B、Phantom A-D、強化Saramandor A-C、強化Gargoyle A/Bの既定値を調整済み設定へ更新した。
- 敵設定画面の見出し画像を整理し、Panel MonsterとSpark Ballは対象の1体だけ、強化Ghostは右向きだけを表示し、Phantom Bulletには専用色を適用した。

## v0.9.46 (2026-07-19) 強化サラマンダー・ガーゴイルの状態初期化を修正
- 強化サラマンダーA/B/Cと強化ガーゴイルA/Bの速度初期化で、原作処理へ戻す前にstate保存先のYオフセットを復元するよう修正した。
- 誤ったRAM位置への書込みと毎フレームのstate再初期化を止め、落下速度の異常と強化ガーゴイルの歩行アニメが先頭フレームへ戻り続ける問題を解消した。

## v0.9.45 (2026-07-19) 完成済み追加敵を通常表示
- 完成済みの追加敵59体を開発者モード対象から外し、新規環境を含む通常設定で全122体をピッカーへ表示するようにした。
- 将来の未完成要素を一時的に隠せるよう、開発者モード自体とピッカーの判定機構は残した。

## v0.9.44 (2026-07-19) お気に入り欄の表示切替を追加
- 設定画面の表示項目から、通常ステージのピッカー下部にあるお気に入り欄をON/OFFできるようにした。
- お気に入り欄を非表示にしても登録内容とショートカットを保持し、Stage 51のボーナスアイテム欄は従来どおり表示する。
- 日本語・英語UIへ同じ設定文言を追加した。

## v0.9.43 (2026-07-19) 設定とUIの安全性を改善
- 横穴侵入安定化の受付フレーム既定値を20から10へ変更した。設定画面では引き続き1～255フレームの範囲で調整できる。
- 共通設定JSONの形式検査を強化し、インポート途中で失敗した場合はROM、画面値、メタ情報、プレビュー、未保存状態、操作ログを開始前へ戻すようにした。
- 設定ファイルを一時ファイルから原子的に置換する保存方式へ変更し、明示的な設定適用で保存できなかった場合は警告するようにした。
- 表示言語の変更時に、開いている設定画面を含むラベル、ボタン、タブ、選択肢、ツールチップを即時に再翻訳するようにした。
- K/P文字タイルの初期書込みをステージ表示処理から分離し、日本版ROMをmapper66形式へ変換する時に一度だけ行うよう整理した。

## v0.9.42 (2026-07-19) 横穴侵入安定化を正式実装
- 横穴の少し上で穴へ向かって方向を押し直すと、設定フレーム中は横穴上端の位相拒否だけを解除するようにした。
- 上下とも壁の通常壁は解除せず、方向を押しっぱなしにした場合は原作どおりの挙動を維持する。
- 受付フレーム数をUIから1～255で設定可能にし、既定値を20フレームにした。
- 試験表記を外し、操作方法の説明とROM/RAM管理簿を最終仕様へ更新した。
...
- Blue Key Queen runtimeを`0x6066-0x6084` / `$E056-$E074`の31Bへ配置し、後続`0x6085-0x60CB` / `$E075-$E0BB`を71Bの空きとして確定した。
...
- Crystal max fire range取得時は火球距離を一時的に`$1F1F`へ変更し、ステージ開始時に基礎アイテム`$1B`の恒久効果`+$04`適用後の成長値へ復元する。
- 通常の飛距離アイテムは、成長上限を原作の`#$02`から意図的に`#$0F`へ引き上げている。Crystalの一時最大値`$1F1F`とは別の上限として扱う。
...
- UIピッカーとステージ描画では黄色オーバーレイを使わず、Dark Fairy専用のパレット違い表示にした。
...
- 通常、隠し、茶ブロック内、白ブロック内、透明ブロック内、ひび割れブロック内の6状態で保存・読戻しできることを確認した。
- ROM管理簿を更新し、Fire/Fairy x2 item runtimeの配置と対象IDを記録した。追加RAMは使わない。

## v0.9.23 (2026-07-05) オールキルモードのitem $19取得後表示残りを修正
- オールキルモードON時、敵全滅アイテム `$19` の取得直後に扉オープン演出が割り込むと、取得したアイテムの表示が消えずに残る問題を修正した。
- item取得後のgroup4 cleanup actionが残っている間は、Enemy Clear Key Open runtimeの発火を待つようにした。
- Enemy Clear Key Open runtimeを107Bへ拡張し、直後緩衝は14Bに更新した。追加RAMは使わない。
- ROM管理簿を更新し、Enemy Clear Key Open runtimeの配置と残り緩衝を記録した。

## v0.9.22 (2026-07-05) オールキルモードを追加
- オールキルモードをステージ設定に追加し、画面上の敵スロットが空になった時に鍵取得相当の扉オープン演出へ接続するようにした。
- オールキルモードON時は鍵削除を許可しつつ、鍵削除後に扉が残る場合の保存前警告は従来通り出すようにした。
- オールキルモードと鍵持ち敵は同じ開始画面アナウンス枠を共有するため、同時ONをUIで拒否するようにした。
- 開始画面アナウンスに `ALL KILL` を追加し、8項目表示の左右列を1タイル左へ寄せた。
- ROM管理簿を更新し、All Kill runtime、StageExt bit、PRG1 helper、開始画面アナウンス拡張後の配置を記録した。
...
- Ice Flameの足場を崩した時にWhite Flame系の挙動へ寄るよう、runtimeを117Bへ更新した。PRG0の使用範囲は `$E9C1-$EA35` / file `0x69D1-0x6A45`。
- 敵ピッカーではRed Flame / White Flame / Ice Flameをリスト末尾へまとめ、Ice Flame配置表示の水色フィルターを外した。
- ROM管理簿と新敵ID分類フロー文書を更新し、Ice Flame runtimeの使用範囲、緩衝24B、残り空き1,471Bを記録した。
...
- 保存時runtimeは従来どおり必要な領域へ再注入し、初期化済みのPRG0空き状態を保存処理が受け入れるようにした。

## v0.9.15 (2026-06-30) 敵の共通移動速度を整理
- 「ゴーレム/ドラゴン/ガーゴイル歩行速度」を「ゴーレム/ドラゴン/ガーゴイル移動速度」に改め、3匹の普通の移動とゴーレム特攻速度が同じ倍率で変わるようにした。
- ゴーレムの隠し扱いだった古い速度設定コードを削除し、現在UIで使う移動速度設定だけに整理した。
- Panel Monsterの未使用だった発射前ディレイ短縮コードを削除し、現行runtimeでは原作通りの発射前待ちを明示して使うように整理した。

## v0.9.14 (2026-06-30) ゴーレムとデーモンヘッドのキビキビ動作を整理
- ゴーレムのキビキビ動作は、初期待ち/起動待ちを対象外にし、視認後復帰待ちと方向転換待ちだけを短縮するようにした。
- デーモンヘッドのキビキビ動作ラベルを「方向転換の待ち」に統一し、実際の挙動に合わせた。
- 敵設定画面の重複アイコン表示を抑え、ドラゴン/サラマンダーなど必要な敵アイコンを表示するようにした。

## v0.9.13 (2026-06-30) ドラゴン/サラマンダーの火吐き開始待ちを追加
- ドラゴンとサラマンダーに共通する火吐き開始待ちを、敵設定画面から変更できるようにした。
- ドラゴンの既存キビキビ動作ラベルを「方向転換」に整理し、新しい火吐き開始待ち設定と区別しやすくした。
- タイトル文字編集の文字数表示とスペース扱いを調整し、入力中の文字数を確認しやすくした。

## v0.9.12 (2026-06-30) ファイル選択フォルダーを用途別に記憶
- ファイル/フォルダー選択ダイアログの初期フォルダーを、ROM、IPS、ステージPNG、タイトル画像、共通設定JSON、ROM比較、CSV、パレット、16x16画像、設定画面など用途別に記憶するようにした。
- 保存先と読み込み元が別用途で混ざりにくくなるよう、最後に使ったフォルダーを `last_file_dialog_dirs` に種類別保存するようにした。

## v0.9.11 (2026-06-30) 強化ガーゴイルの弾挙動を設定化
- 強化ガーゴイルの同時2発発射を廃止し、原作同様に1発だけ撃つ方式へ戻した。2発同時発射は当初有効に見えたが、実際には効果が薄く見た目の挙動も不自然だったため、弾そのものの速度と発射間隔を設定できる方式へ変更した。
- 7A/7B と 7E/7F それぞれのBullet速度を 1/4・1/2・2x・3x から選べるようにし、7A/7B はクールダウンも設定できるようにした。7E/7F のクールダウンは0F固定。
- 通常ガーゴイルのBullet速度マーカー消去分岐を修正し、強化ガーゴイル設定の影響が通常IDへ残らないようにした。
- Panel Variant final適用時にパネルモンスター検出が誤警告になるケースを修正した。

## v0.9.10 (2026-06-28) Undo/Redo状態ボタンを追加
- Undo一覧の右に小さい戻る/進むボタンを追加し、Undo/Redoできる状態を一覧を開かず確認できるようにした。

## v0.9.9 (2026-06-28) Undo一覧に初期状態を表示
- Undo一覧にROM読込直後の初期状態を常に表示し、履歴がない状態でも一覧から基準点を確認できるようにした。

## v0.9.8 (2026-06-28) Undo一覧ウィンドウ位置を復元
- Undo一覧ウィンドウの位置とサイズを保存し、次回表示時に復元するようにした。

## v0.9.7 (2026-06-28) 無変更配置のUndo記録を抑制
- 1マス配置で操作前後のマス状態が変わらない場合、Undo履歴を追加しないようにした。
- 鍵の位置へ通常白ブロックを置こうとした場合、内部データへ白ブロックを書き込む前に拒否するようにした。
- 原作JP ROMを初回読み込みでmapper66化した直後、編集用ステージデータもmapper66形式から読み直して正規化するようにした。

## v0.9.6 (2026-06-28) Undo履歴一覧とジャンプを追加
- メイン画面に `Undo一覧` ボタンを追加し、Undo/Redo履歴を時刻・対象ステージ・操作・座標/詳細つきで一覧表示できるようにした。
- 履歴一覧は開いたまま編集できる非モーダル表示にし、行をダブルクリック、または選択して移動ボタンを押すと、その履歴位置へUndo/Redoを連続実行してジャンプできるようにした。
- 新しく積むUndo履歴へ、配置・削除・ステージクリアなどの操作名と座標情報を記録するようにした。配置/削除では対象名、単品移動では移動対象名と移動元/移動先も表示するようにした。
- Undo履歴に通番を付け、履歴上限で古い項目が消えても残った履歴の番号が詰まらないようにした。
- 履歴一覧のステージ列を `面` 表示に変更し、連続配置/連続削除は詳細羅列ではなく個数で表示するようにした。
- 履歴一覧を開いたまま編集して履歴上限に達しても、表示更新が重くなりにくいようにした。

## v0.9.5 (2026-06-28) 別ROMタイトル取り込みを整理
- タイトル画面の別ROM取り込みで、重複していた `transcode_title` 定義を削除。
- US arcade title hack 専用の取り込み経路を削除し、通常US/JP系の取り込みへ整理。
- 別ROMからタイトルを取り込む時、上部256x64範囲の色区分とタイトルBGパレットも移植元から反映するようにした。

## v0.9.4 (2026-06-28) ステージ制限の全53面適用を追加
- ステージ設定のBボタン禁止、Aボタン禁止、暗闇モード、開始時ファイヤーリセットへ右クリックメニューを追加。
- 右クリックから、現在の項目のチェック状態を全53面へ適用できるようにした。

## v0.9.3 (2026-06-28) ショートカット表示名の英語対応
- F1ショートカットマップのキー内表示とマウスホバー説明を、表示言語に合わせて英語表示するようにした。
- 設定画面のショートカット一覧と重複警告でも同じ表示名を使うようにした。

## v0.9.2 (2026-06-28) 初回言語選択に再設定案内を追加
- 初回起動時の言語選択ダイアログに、あとで設定画面 (F9) から表示言語を変更できる案内を追加した。

## v0.9.1 (2026-06-28) 初回起動時の言語選択を追加
- 設定ファイルに表示言語が保存されていない初回起動時、英語/日本語を選ぶダイアログを表示するようにした。
- 既存ユーザーの保存済み表示言語は変更しない。
...
## v0.8.407 (2026-06-27) 英語表示の未翻訳UIを追加対応
- ゲーム挙動改造ダイアログの主要グループ、説明、適用結果、失敗タイトル、インポート変更項目を英語表示に対応させた。
- ショートカットヘルプ、ピクセル編集、ファイル選択、タイトル画面編集の一部に残っていた日本語のステータスやボタン文言を翻訳キーへ移した。
- タイトル画面編集のタイトル/クリア画面/エンディング各タブ冒頭の説明と主要操作ボタンを英語表示に対応させた。

## v0.8.406 (2026-06-27) エンディングメッセージを自動中央寄せ
- エンディング文字列が固定文字数より短い場合、前後にスペースを足して中央寄せで書き込むようにした。

## v0.8.405 (2026-06-27) クリア画面メッセージを自動中央寄せ
- クリア画面メッセージが固定文字数より短い場合、前後にスペースを足して中央寄せで書き込むようにした。

## v0.8.404 (2026-06-27) クリア画面プレビューの色と倍率操作を修正
- クリア画面プレビューで空白タイルを白く描いていた問題を修正し、文字本体を白で表示するようにした。
- クリア画面プレビューでも Ctrl+マウスホイールで表示倍率を変更できるようにした。
- クリア画面プレビューでは、文字確認を妨げるグリッド線を表示しないようにした。

## v0.8.403 (2026-06-27) クリア画面メッセージの簡易プレビューを追加
- タイトル画面編集の `クリア画面` タブで、3行メッセージを直接編集できるようにした。
- 実機エミュレーションではなく、茶色背景に原作フォントで3行を置く簡易プレビューを追加した。

## v0.8.402 (2026-06-27) クリア画面設定タブを追加
- タイトル画面編集に `クリア画面` タブを追加し、既存のクリア画面キャラ差し替えを移した。
- 同じタブから既存のクリア画面メッセージ編集を開けるようにした。
- ゲーム挙動改造側に残っていた同じクリア画面UIは非表示にした。

## v0.8.400 (2026-06-27) エンディング条件表示を追加
- エンディングタブの表示プルダウン横に、選択中エンディングの条件を表示するようにした。

## v0.8.399 (2026-06-27) エンディング編集欄の表示を分岐別に整理
- エンディングタブ右側の文字列編集欄を、選択中のエンディングで表示される行だけに絞るようにした。
- 編集欄の並びを、画面上に出る文の順番に合わせた。

## v0.8.398 (2026-06-27) 山なしエンディングの黒背景を修正
- `True Ending` と `Bad Ending A` のプレビューで、黒い全面塗りではなく空背景タイルを全面に敷くようにした。

## v0.8.397 (2026-06-27) 山なしエンディング背景を修正
- `True Ending` と `Bad Ending A` のプレビューでは背景ストリームを描かず、共通背景色で全面を塗るようにした。
- エンディングタイルの透明色相当を共通背景色で描き、下段にタイトル床色のオレンジが出ないようにした。

## v0.8.396 (2026-06-27) 王女あり固定文の編集欄を追加
- エンディング文字列編集に `Fairy 1` / `Fairy 2` を追加し、王女あり分岐の固定2行も編集できるようにした。
- 王女ありTrue Endingプレビューが黒くなる原因だった空タイル背景生成をやめ、通常背景の空タイルで画面を埋めるようにした。
- エンディングプレビュー上で Ctrl+マウスホイールにより表示倍率を変更できるようにした。

## v0.8.395 (2026-06-27) 王女ありエンディング表示を追加
- タイトル画面編集のエンディングタブで、`True Ending` / `Good Ending A` / `Good Ending B` を追加し、6分岐の表示切替にした。
- 王女あり分岐では、ROM上の固定文 `FAIRIES WERE RELEASED` と `AND FLEW AROUND THE WORLD` もプレビューへ描画するようにした。
- デフォルト表示は一番下の `Worst Ending` のままにした。

## v0.8.394 (2026-06-27) エンディング表示プルダウン名を整理
- タイトル画面編集のエンディングタブで、既存3分岐を `Bad Ending A` / `Bad Ending B` / `Worst Ending` として表示するようにした。
- デフォルト表示を `Worst Ending` に変更した。

## v0.8.393 (2026-06-27) True Endingプレビューの行位置を修正
- True Endingプレビューで、`AND` 以降の専用3行が共通文より上に出ていた問題を修正した。
- True専用3行はプレビュー上だけ12タイル行下げ、実機表示と同じ並びにした。

## v0.8.392 (2026-06-27) エンディング引用符タイルの編集を修正
- エンディング文字列で `0x28` と `0x3A` を `"` として読み、末尾の引用符を `.` と誤表示しないようにした。
- `"` の書き戻しは元のタイル位置に応じて `0x28` / `0x3A` を維持し、ファミコン側の引用符表示を壊さないようにした。
