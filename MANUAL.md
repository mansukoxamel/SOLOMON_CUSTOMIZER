...
SOLOMON_CUSTOMIZERは、『ソロモンの鍵』の日本ファミコン版・US NES版オリジナルROMを、ステージ編集の範囲を超えて改造するためのGUIツールです。
オリジナルROMを読み込むと、編集用の mapper66 / wide-title 拡張形式へ自動変換し、ステージ配置、敵、部屋ごとの特殊ルール、タイトル画面、パレット、ゲーム挙動、テストプレイ用ROM生成までまとめて扱えます。
...
- 対応する原作ROMは、日本版（CRC32 `013ED497` / `5B49FEDB` / `2FE9E2CA`）とUS版（CRC32 `B7A00D99` / `99773BC4`）です。US版は、ヘッダーだけが異なりPRG CRC32 `0771C34F`・CHR CRC32 `FAD8A464`が一致する原本も受け付けます。
- `.nes`を直接読み込めます。`.zip`の場合は、中に対象ROMが入っているものを読み込めます。
- 通常の編集対象は、オリジナルROMから変換された mapper66 / wide-title 拡張ROMです。
- US版オリジナルROMは、読み込み時に日本版相当の内部配置へ正規化してから同じ編集処理を行います。
...
1. `ROMを開く`で、対応する日本版またはUS版オリジナルROM（`.nes` / `.zip`）を読み込みます。
...
`タイトル画面編集`では、読み込み時にmapper66 / wide-title内部形式へ正規化されたタイトル画面を編集します。
日本版・US版のどちらを読み込んだ場合も、同じ編集画面を使用します。
...
| 項目 | 内容 |
|---|---|
| 強化サラマンダー（火球発射化） | 移動速度1固定。炎速度を通常・1/2・1/4から設定。最低歩行時間は発射後停止を終えて歩き始めてから次の対ダーナ攻撃が可能になるまでの1～255フレームで、原作値は32。ダーナが範囲外なら設定時間後も歩き続ける。発射後停止時間は炎が口元に出てから歩き出すまでの28～231フレームで、原作値は28 |
| パネルモンスター | クールダウン、弾の左右速度バグ修正、修正後速度を設定 |
| ゴブリン | ゴブリン関連の待ちや挙動を調整 |
| ゴブリン/ドラゴン/ガーゴイル歩行速度 | 共有歩行速度を倍率変更 |
...
| 強化ガーゴイルA/B | 両方とも移動速度1。A/B別に弾速・発射間隔・クールダウンを設定。両方ともLIFE百の位で2発/3発を切替 |
| ドラゴン | 攻撃前の待ちを最小化 |
...
- 対応するUS版オリジナルROM以外の別リージョン・改造済みUS ROMは編集対象外です。
