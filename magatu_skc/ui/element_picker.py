...
ITEMS_LIST = [
    0x04, 0x05, 0x08, 0x0c, 0x36,
    0x11, 0x12, 0x13, 0x14, 0x15, 0x34, 0x16, 0x17, 0x18, 0x35, 0x1a, 0x19,
    0x1b, 0x0b, 0x3a, 0x1c, 0x1d, 0x1e, 0x1f, 0x20, 0x21, 0x22, 0x25, 0x26,
    0x27, 0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x2d, 0x2e, 0x2f, 0x30,
    0x31, 0x32, 0x33, 0x37, 0x38, 0x39,
]

SPECIAL_ITEM_PICKER_CODES = {0x34, 0x35, 0x36, 0x3a}
...
ENEMIES_LIST = [
    ...
    (0x7b, "Enhanced Gargoyle A (left)"),
    (0x7e, "Enhanced Gargoyle B (right)"),
    (0x7f, "Enhanced Gargoyle B (left)"),
    (0x50, "Demonhead (R)"),
    (0x51, "Demonhead (L)"),
    (0x5c, "Saramandor (R)"),
    (0x5d, "Saramandor (L)"),
...
ENEMY_ENHANCE_CYCLES = (
    ...
    (0x78, 0x7a, 0x7e), (0x79, 0x7b, 0x7f),
    # Panel Monster: normal -> 2-way -> 3-way -> A -> B -> C -> D -> normal.
    (0x24, 0xF0, 0xF4, 0xE0, 0xE4, 0xE8, 0xEC),
    (0x25, 0xF1, 0xF5, 0xE1, 0xE5, 0xE9, 0xED),
    (0x26, 0xF2, 0xF6, 0xE2, 0xE6, 0xEA, 0xEE),
    (0x27, 0xF3, 0xF7, 0xE3, 0xE7, 0xEB, 0xEF),
)


ENEMY_ENHANCE_NEXT = {
    code: cycle[(idx + 1) % len(cycle)]
...
def enemy_enhance_variant(code: int) -> int | None:
    """Return the next enhanced/alternate enemy code for K-toggle, or None."""
    return ENEMY_ENHANCE_NEXT.get(int(code) & 0xFF)
...
class ElementPicker(QWidget):
            ...
            for key, label, val in [
                ...
                if key == "door":
                    self._add_picker_item(
                        ...
                        MODE_ITEM,
                        0x07,
                        item_picker_label(0x07, self.config),
                        self._make_item_icon(0x07),
                    )
    ...
    def _update_flag_controls(self, mode, value):
        ...
        if mode == MODE_META and value == "start":
            allowed = {ITEM_FLAG_NORMAL}
        elif mode == MODE_META and value in ("mirror1", "mirror2"):
            allowed = set(all_flags) - {ITEM_FLAG_HIDDEN}
