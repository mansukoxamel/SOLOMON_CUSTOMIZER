...
_TRANSLATIONS = {
    ...
    "hack_dialog.saramandor_variant.hint": {
        "ja": "炎速度、最低歩行時間、発射後停止時間は、強化サラマンダー $5E/$5F だけに適用します。最低歩行時間は1～255、原作32です。初回発射は原作32のまま、発射後停止を終えて歩き始めた後の2回目以降だけ設定値を適用します。ダーナが範囲外なら設定時間を過ぎても歩き続けます。発射後停止時間は炎が口元に出てから歩き出すまでの時間で、28～231、原作28です。",
        "en": "Flame speed, minimum walk time, and post-fire stop apply only to Enhanced Saramandor $5E/$5F. Minimum walk time ranges from 1 to 255, with stock at 32. The first attack keeps the stock wait; the setting applies to later attacks after post-fire stopping ends and walking begins. If Dana is outside the attack range, it keeps walking after the configured time. Post-fire stop is measured from the flame appearing at the mouth until walking resumes; its range is 28-231 and stock is 28.",
    },
    ...
    "hack_dialog.gargoyle_variant.hint": {
        "ja": "A/Bとも移動速度1です。どちらもLIFE百の位が偶数なら2発、奇数なら3発撃ちます。弾速・発射間隔・発射後クールダウンはA/B別に設定できます。",
        "en": "Both A and B use movement speed 1. Each fires twice when the LIFE hundreds digit is even and three times when it is odd. Bullet speed, shot interval, and post-shot cooldown are configured independently for A and B.",
    },
