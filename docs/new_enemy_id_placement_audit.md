...
| Spark85 / 停止後反転型 | `$85` | 専用ID | Spark系24連番の後半8IDへ統合・番号保留 | 監査中 | 未検査 | 4方向×2速度、借用Spark群との統合 |
...
| Neul Twin Cannon | `$88` | 専用ID | 上下2連番へ再配置・番号保留 | 静的監査継続 | 現行上移動・左右壁破壊はユーザー確認済み | 下向き初期化、壁反転、実ROM比較 |
...
|---:|---|---:|---|
| `+0` | Neul Twin Cannon上 | 現行成立値を維持 | 右・左の2発 |
| `+1` | Neul Twin Cannon下 | 初期Y速度を含めて要設計 | 右・左の2発 |
...
下向き版は単純にbehavior `$03` を書けば完成すると未検査のまま決めない。setupで設定される初期Y速度、state 0以降の方向更新、天井・床接触後の反転まで追って、上下2IDの最小差分を確定する。
...
| setup | `$88` をgroup `$18` 読込へ分岐 | 上下2IDを同じNeul groupとして分類 |
| init | `$88` を専用initへ分岐 | 2IDを範囲分類し、初期Y速度とbehaviorの両方を整合させる |
| animation | 専用分類なし。原作 `$8789` を使用 | 専用分類なしを維持する候補 |
...
- group `$18`、原作Neul上・速度1 `$30` と同じvisual/velocity classを使用する。
- `$D9D3,Y` から原作Neul用metadataを読む。
- 原作UI上ではNeul上 `$30` と下 `$32` のanimation番号はいずれも43であり、上下で専用animation runtimeを分ける必要はない候補である。

#### init

- status `$C0` を設定し、原作init writer `$9D1C` を呼ぶ。
- sub-slot `[7]` を射撃cooldown `$80` で初期化する。
- 現行コードはbehavior `$00` を設定し、コメントではstock `$30` Neul behaviorとしている。
- 原作方向表だけを見ると `$00` は右、`$02` は上だが、現行 `$88` はsetup group `$18` によるY速度設定と組み合わせて実際には上移動している。
- したがって `$00` を直ちにロジックミスとは扱わない。コメントと実際の成立理由が不足している状態として扱う。
- 下向き版はbehaviorだけでなく初期Y速度も含めて設計し、床・天井での反転まで実機確認する。
...
- 現時点で現行上向きをロジックミス扱いしない。
- 一方で、原作方向表と専用initコメントだけでは成立理由が分かりにくい。初期Y速度設定とstate遷移を含む説明を実装時に正す必要がある。
...
| 現行保存ROMの本体バイト列比較 | 未検査 | 現行完成形ROMを新規出力して比較が必要 |
| 現行上向き `$88` | ユーザー実機確認済み | 正しく上移動。成立理由の完全な静的説明は継続調査 |
| 新規下向き版 | 未作成・未検査 | 初期Y速度、behavior、表示、射撃、壁反転を確認する |
| child slot不足時の右弾優先 | 仕様確定 | 空き1slotなら右弾だけ。修正しない |
| 親死亡時の左右弾 | 未検査 | 独立Bulletとして継続する仕様でよいか確認する |
...
- 速度違い・追加派生: 作らない
- 配置: 2連番へ再配置する
- 連番内規則: 2連番を上・下の順にする。bit 0だけで実装できるかは初期Y速度解析後に確定
- 開始ID: 全体配置図まで保留
- 確保範囲: 必要数は2ID。2IDだけ詰めるか未使用枠を隣接させるかは全体配置図まで保留
- 現行 `$88`: 完成形の2ID配置が決まるまで仮位置
- 入口センター: AI/setup/initを2ID範囲として分類、animation専用分類は不要候補
- runtime: 現行上移動を維持し、下向き用の初期Y速度とbehaviorを確定する。Ghost系との射撃共通化も検討する
- 2発child処理: sub-slot `[6]` は一時受渡しなので2発生成と矛盾しない。slot不足時の右弾優先は許容仕様
- 鍵持ち敵仕様: 上下とも指定可能候補。火球撃破と鍵出現を検査する
- 実装状態: 未変更
...
- 停止後反転型: 停止条件へ入った時に一度だけ進行方向を逆へ切り替え、その後停止する。現行Spark85の挙動を4方向へ拡張する。

### 方向と速度

- 各種類は右・左・上・下の4方向を持つ。
- 方向順は `0=右 / 1=左 / 2=上 / 3=下` を基本候補とするが、Spark Ball内部のdirection/phase fieldはBulletと異なるため、実装前に原作Spark専用方向encodeへ変換する。
- 速度1/2は別IDグループとして維持する。
- 各速度グループの具体的な速度倍率はUI設定で指定し、ROM作成時に速度tableまたは即値へ反映する。
...
- 原作Dragon/Golem/Spark Ballへのtype-specific hookを減らせるか、共通入口センターへどう統合するかは続くruntime監査で確定する。
...
- 停止型: そのまま停止する。
- 停止後反転型: entityごとのone-shot flagを確認し、未反転ならSpark専用direction/phase fieldを一度だけ反転してから停止する。
