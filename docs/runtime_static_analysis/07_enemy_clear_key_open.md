...
確定問題2件は修正済みである。全滅走査では`AND #$FD / CMP #$81`により撃破不能な`$81/$83`を除外し、`$9D`と同様に残存していても全滅を妨げないようにした。旧111B版・旧114B版runtimeの受入れ経路は削除し、空きまたは現行runtimeだけを正常入力として認める。

この修正により本体は6B増加し、130Bになった。以下の「確定した問題」は修正前の解析結果と原因を記録したものである。
...
| file | CPU | size | 内容 |
|---:|---:|---:|---|
| `0x4156-0x41D7` | `$C146-$C1C7` | 130B | Enemy Clear Key Open本体 |
| `0x1F19-0x1F1B` | `$9F09-$9F0B` | 3B | main-loop tail hook |

直後`0x41D8-0x420F`は56Bの正式な空きである。本runtimeの予約は130Bだけで、正式ROM管理簿と`RESERVED_SPANS`が一致する。
...
- runtime 130B、hook 3B、正式ROM/RAM管理簿との一致
